...
const toWebSocketUrl = (apiBase, path, session) => {
 ...
 */
export function createServerConnection(tx, sx = {}) {
    ...
    const emitConnectionError = async (detail) => {
        await tx.send('connectionError', detail);
    };
    const attachSocket = (instance) => {
    ...
    return {
        async onAuthenticateUser(payload) {
