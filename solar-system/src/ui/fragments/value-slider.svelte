<script>
    ...
    function onInput(e) {
        ...
        // Now it is either a number or a an empty string
        if (input.value === "") {
        ...
        // the value still has to be input
        if ((inputValue == 0) && (range.min > 0)) return
    ...
    function updateSliderPosition() {
        ...
        if (slider && (range.current !== "")) {
            ...
            slider.value = Math.max(range.min, Math.min(range.max, range.current));
        }
    }
</script>
...
<div class="value-slider">
    ...
    <input bind:this={slider} style={style} class="slider" type="range" min={range.min} max={range.max} on:input={onSliderInput}>
</div>
